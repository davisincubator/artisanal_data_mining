...
def get_batch(train_files,batch_size):
  f_batch = np.random.choice(train_files,batch_size)
  imgBatch = np.zeros(shape=(batch_size, 256, 256, 4))
  for b,f in enumerate(f_batch):
    img = cv2.imread(f, cv2.IMREAD_UNCHANGED)
    img = img.astype(np.float32) / 255.
    imgBatch[b, :, :, :] = img
  fileNames = [file_parts(x)[1] for x in f_batch]
  return fileNames,imgBatch

def get_labels(cat_dict,labelList):
  labels = np.zeros([len(labelList),len(cat_dict)])
  for i in range(len(labelList)):
    labels[i,[cat_dict[l] for l in labelList[i].split()]] = 1
  return labels
# file io functions
